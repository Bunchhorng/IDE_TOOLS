...
const BRANDS: Record<Exclude<LangGlyph, 'file'>, BrandGlyph> = {
  c: {
...
export function LanguageIcon({
  ...
  const icon = { sm: 12, md: 17, lg: 22 }[size];
  ...
  const g = BRANDS[lang];
  return (
    <span className={cn('inline-flex shrink-0 items-center justify-center', g.bg, box, className)}>
      <svg width={icon} height={icon} viewBox="0 0 24 24" fill="currentColor" aria-label={g.label} role="img">
        <path d={g.path} />
      </svg>
    </span>
  );
